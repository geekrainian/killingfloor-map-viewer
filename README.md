...
A few rough edges are listed at the end of [`RESEARCH.md`](./docs/RESEARCH.md). Notably: meshes whose `.usx` isn't installed are skipped (shown in the log), and terrain height uses the standard UE2 mapping, so if a map's terrain looks too flat or too tall the divisor in the code is the knob.
