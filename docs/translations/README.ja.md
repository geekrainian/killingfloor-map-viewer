...
- 下記の **デスクトップアプリ**、または最近の Chromium ベースのブラウザ（Chrome / Edge）のいずれか。ビューアはフォルダ選択と S3TC 圧縮テクスチャ拡張を使います。
...
1. デスクトップアプリを開くか、ブラウザで `viewer.html` を開きます（ダブルクリック）。オフラインで動作し、three.js は `vendor/` に同梱されています。
...
| `electron/main.js` | `viewer.html` をデスクトップアプリとしてホストする Electron シェル |
| `vendor/three.min.js` | three.js r136（ビューアがオフラインで動くよう同梱） |
| `RESEARCH.ja.md` | `.rom` フォーマットとビューアの描画方法に関するノート |

## ライセンス

このプロジェクトは **GNU General Public License v3.0**（`GPL-3.0-only`）の下でライセンスされています。全文は [`LICENSE`](../../LICENSE) を参照してください。
